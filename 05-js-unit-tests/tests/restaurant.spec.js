...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(meuRestaurante.fetchMenu()).toEqual({ food: {}, drink: {} })
    ...
    // ```
    const meuMenu = {
      food: { 'coxinha': 4.90, 'sanduiche': 9.90},
      drinks: { 'agua': 3.90, 'cerveja': 6.90 }
    };
    meuRestaurante = createMenu(meuMenu);
    expect(meuMenu).toEqual(meuRestaurante.fetchMenu());
    ...
    // ```
    expect(meuRestaurante.pay()).toEqual(((4.9 * 2) + 3.90) * 1.1);
